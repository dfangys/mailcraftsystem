...
class SecurityAuditService {
  ...
  Future<SecurityAuditReport> performSecurityAudit() async {
    ...
    try {
      ...
      AppLogger.info('Security audit completed in ${duration.inMilliseconds}ms');
      return report;
    } catch (e) {
    ...
    try {
      ...
      if (Platform.isAndroid) {
        ...
        if (androidInfo.version.securityPatch != null) {
          final patchDate = DateTime.tryParse(androidInfo.version.securityPatch!);
          if (patchDate != null) {
  ...
  SecurityRiskLevel _calculateOverallRisk(List<SecurityFinding> findings) {
    ...
    final highCount = findings.where((f) => f.severity == SecuritySeverity.high).length;
    if (highCount >= 3) {
    ...
    final mediumCount = findings.where((f) => f.severity == SecuritySeverity.medium).length;
    if (mediumCount >= 5) {
      return SecurityRiskLevel.medium;
    }
