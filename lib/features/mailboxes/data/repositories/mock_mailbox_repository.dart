...
class MockMailboxRepository implements MailboxRepository {
  ...
  @override
  Future<Either<Failure, Mailbox>> getMailbox(
      String accountId, String path) async {
    await Future.delayed(const Duration(seconds: 1));
    return Right(Mailbox(
        id: path,
        path: path,
        name: path,
  ...
  @override
  Future<Either<Failure, List<Mailbox>>> refreshMailboxes(
  ...
  Future<Either<Failure, Mailbox>> createMailbox(
      String accountId, String name, String? parentPath) async {
    await Future.delayed(const Duration(seconds: 1));
    return Right(Mailbox(
        id: name,
        path: name,
  ...
  Future<Either<Failure, void>> renameMailbox(
      String accountId, String currentPath, String newName) async {
    await Future.delayed(const Duration(seconds: 1));
  ...
  Future<Either<Failure, MailboxStatus>> getMailboxStatus(
    ...
    return Right(MailboxStatus(
        path: path, messageCount: 10, unreadCount: 2, recentCount: 0));
  }
  ...
  @override
  Future<Either<Failure, List<Mailbox>>> getMailboxHierarchy(
      String accountId) async {
    return getMailboxes(accountId);
  }
