...
class MailClientFactory {
  ...
  static Future<ImapConnectionResult> testImapConnection(
    ...
    try {
      AppLogger.info(
          'Testing IMAP connection to ${config.imapConfig.host}:${config.imapConfig.port}');

      await client.connect();

      // Get capabilities
      final imapClient =
          client.lowLevelIncomingMailClient as enough_mail.ImapClient;
      final capabilities = imapClient.serverInfo.capabilities;

      final capabilityNames = capabilities?.map((c) => c.name).toList() ?? [];
      AppLogger.info(
          'IMAP connection successful. Capabilities: ${capabilityNames.join(', ')}');
  ...
  static Future<SmtpConnectionResult> testSmtpConnection(
    ...
    try {
      AppLogger.info(
          'Testing SMTP connection to ${config.smtpConfig.host}:${config.smtpConfig.port}');
  ...
  static Future<FullConnectionResult> testFullConnection(
    ...
    AppLogger.info(
        'Testing full connection for account: ${config.displayName}');
...
class FullConnectionResult {
  ...
  String get statusSummary {
    ...
    if (imapResult.success && !smtpResult.success)
      return 'IMAP connected, SMTP failed';
    if (!imapResult.success && smtpResult.success)
      return 'SMTP connected, IMAP failed';
